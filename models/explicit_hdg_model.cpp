...
explicit_hdg_model<dim, CellType>::explicit_hdg_model(
 ...
 */
template <int dim, template <int> class CellType>
void explicit_hdg_model<dim, CellType>::init_mesh_containers()
{
 ...
 */
template <int dim, template <int> class CellType>
void explicit_hdg_model<dim, CellType>::set_boundary_indicator()
{
 ...
 */
template <int dim, template <int> class CellType>
void explicit_hdg_model<dim, CellType>::count_globals()
{
       ...
       ++ghost_cell_it)
  {
    ...
    {
      if ((*ghost_cell_it)->face_owner_rank[i_face] == comm_rank)
      {
        for (unsigned i_dof = 0;
             ...
             ++i_dof)
        {
          ...
          all_owned_dofs[dof_i1].connected_face_of_parent_ghost.push_back(i_face);
          if (all_owned_dofs[dof_i1].n_local_connected_DOFs == 0)
          {
      ...
      for (unsigned j_face = 0; j_face < n_faces_per_cell; ++j_face)
      {
        ...
        if (j_face != face_ij)
          for (unsigned i_dof = 0;
               ...
               ++i_dof)
          {
            if ((*parent_cell)->face_owner_rank[j_face] == comm_rank)
              local_dofs_num_map[(*parent_cell)->dofs_ID_in_all_ranks[j_face][i_dof]]++;
            else
              nonlocal_dofs_num_map[(*parent_cell)->dofs_ID_in_all_ranks[j_face][i_dof]]++;
          }
      }
    }
      ...
      for (unsigned j_face = 0; j_face < n_faces_per_cell; ++j_face)
      {
        ...
        if (j_face != face_ij)
          for (unsigned i_dof = 0;
               ...
               ++i_dof)
          {
            if ((*parent_ghost)->face_owner_rank[j_face] == comm_rank)
              local_dofs_num_map[(*parent_ghost)->dofs_ID_in_all_ranks[j_face][i_dof]]++;
            else
              nonlocal_dofs_num_map[(*parent_ghost)->dofs_ID_in_all_ranks[j_face][i_dof]]++;
          }
      }
    }
...
template <int dim, template <int> class CellType>
void explicit_hdg_model<dim, CellType>::free_containers()
{
...
template <int dim, template <int> class CellType>
bool explicit_hdg_model<dim, CellType>::check_for_next_iter(
  ...
  unsigned thread_id = 0;
  {
         ...
         i_cell = i_cell + this->manager->n_threads)
    {
      ...
      this_iter_increment += static_cast<CellType<dim> *>(cell.get())
                               ->get_trace_increment_norm(local_uhat);
...
template <int dim, template <int> class CellType>
bool explicit_hdg_model<dim, CellType>::compute_internal_dofs(
  double *const local_uhat)
{
...
#pragma omp parallel
         ...
         i_cell = i_cell + this->manager->n_threads)
    {
      ...
      static_cast<CellType<dim> *>(cell.get())
        ->compute_internal_dofs(
          local_uhat, u_vec, q_vec, the_elem_equal_dist_basis);
  ...
  double global_u_error, global_q_error;
  MPI_Reduce(
...
template <int dim, template <int> class CellType>
void explicit_hdg_model<dim, CellType>::init_solver()
{
...
template <int dim, template <int> class CellType>
void explicit_hdg_model<dim, CellType>::reinit_solver(
